...
class Procedura:
    ...
    def procedura1 (self,connection):
...
def genera_procedure(connection):
    ...
    lista_procedure = [method for method in dir(Procedura) if method.startswith('__') is False]

    if lista_procedure is not []:
        ...
        if scelta_proc == "si":
            ...
            scelta_proc2 = input()
            istanza = Procedura()
            metodo1 = getattr(istanza,scelta_proc2)
            metodo1()
